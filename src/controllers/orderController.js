...
export const createOrder = async (req, res) => {
    try {
        ...
        // Fetch products from DB that match these IDs
        ...
        // Check if all product IDs exist in DB
        const existingProductIds = existingProducts.map(p => p.id);
        const missingProducts = productIds.filter(id => !existingProductIds.includes(id));

        if (missingProducts.length > 0) {
            return res.status(400).json({
                error: 'The following product IDs do not exist in the database:',
                missingProducts
            });
        }

        // Validate product names and auto-fetch missing names
        ...
        const newOrder = await prisma.order.create({
            data: {
                ...
                phoneNumber,
                email,
                products: updatedProducts,  // Store as JSON
                status: 'Pending'
            }
        });
...
export const updateOrderDetails = async (req, res) => {
    try {
        ...
        if (products) {
            ...
            // Check if admin is trying to add new products
